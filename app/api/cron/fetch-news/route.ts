...
// Set NEWSAPI_KEY environment variable for automatic updates
...
async function fetchNewsFromAPI(apiKey: string): Promise<NewsArticle[]> {
  ...
  // Second: Search ALL sources (captures local affiliates) - English only
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log(`Cron job completed: Added ${data?.length || 0} new incidents`);
    ...
    console.error('Error in cron news fetch:', error);
    return NextResponse.json(
